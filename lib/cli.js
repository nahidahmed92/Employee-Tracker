...
class CLI {
  ...
  updateEmployeeRole() {
    return pool
      .query(
        `
        SELECT e.id, CONCAT(e.first_name, ' ', e.last_name) AS employee_name, r.id AS role_id, r.title FROM employees e
        JOIN roles r ON r.id = e.role_id;
        `
      )
      .then((employeeResult) => {
        ...
        const roleChoices = employeeResult.rows.map((row) => ({
          name: row.title,
          value: row.role_id, // Use role ID as the value
        }));

        const employeeName = employeeResult.rows.map((row) => row.name);
