...
class CLI {
      ...
      .then((employeeRolesResults) => {
        ...
        return inquirer
          ...
          .then((response) => {
            ...
            const selectedManager = managerChoices.find(
              (manager) => manager.value === newManagerId
            );
            ...
            pool.query(sql, params, (err, result) => {
              ...
              console.log(
                `Added ${newEmployeeLastName}, ${newEmployeeFirstName} as ${newEmployeeRoleName} that is managed by ${newEmployeeManagerName} to database.`
              );
              this.run();
            });
          });
  ...
  updateEmployeeRole() {
    return Promise.all([
      pool.query(`SELECT id, CONCAT(first_name, ' ', last_name) AS employee_name FROM employees`),
      ...
      return inquirer
        ...
        .then((response) => {
          ...
          const sql = 'UPDATE employees e SET role_id = $1 WHERE e.id = $2';
          const params = [employeeId, roleId];

          console.log(response.employeeUpdate);
          ...
          pool.query(sql, params, (err, result) => {
            ...
            console.log(`Updated ${employeeName} to be a(n) ${roleName}`);
          });
        });
    });
  }
